...
!>\file mp_udm.F90
!!
!! This file is the mp_udm micorphysics shceme.
...
!-------------------------------------------------------------------------------
!
!! \section arg_table_mp_udm_init Argument Table
!! \htmlinclude mp_udm_init.html
!!
!-------------------------------------------------------------------------------
...
!
      end subroutine mp_udm_init
!-------------------------------------------------------------------------------
!
!
!-------------------------------------------------------------------------------
      subroutine mp_udm_finalize()
!-------------------------------------------------------------------------------
      end subroutine mp_udm_finalize
!-------------------------------------------------------------------------------
!
!
!-------------------------------------------------------------------------------
!> \section arg_table_mp_udm_run Argument Table
!! \htmlinclude mp_udm_run.html
!!
subroutine mp_udm_run(ncol, nlev, t1, q1 &
...
!
   ...
   real(kind=kind_phys)                 , intent(in   ) :: g, cpd, cpv, t0c,   &
                                                             rd, rv, ep1, ep2
   integer, dimension( ncol )         , intent(in   ) :: slimsk
   ...
   real(kind=kind_phys), dimension( ncol,nlev ), intent(in   ) :: delz1
   real(kind=kind_phys), dimension( ncol,nlev ), intent(in   ) :: den1
   ...
   real(kind=kind_phys), dimension( ncol,nlev ), intent(inout) :: qc1, qr1
   real(kind=kind_phys), dimension( ncol,nlev ), intent(inout) :: &
                                                           qi1, qs1, qg1, qh1
   real(kind=kind_phys), dimension( ncol,nlev ), intent(inout) :: &
                                                                nn1, nc1, nr1
   real(kind=kind_phys), dimension( ncol,nlev ), intent(in) :: &
                                                                     w1, tke1
   real(kind=kind_phys), dimension( ncol,nlev ), intent(inout) :: &
                                                                        nifa1
   real(kind=kind_phys), dimension( ncol )          , intent(inout) :: rain
   real(kind=kind_phys), dimension( ncol ), optional, intent(inout) :: &
                                                                         snow, &
                                                                      graupel, &
                                                                         hail
   real(kind=kind_phys), dimension( ncol )          , intent(inout) :: &
                                                                      rainncv, &
                                                                           sr
   real(kind=kind_phys), dimension( ncol ), optional, intent(inout) :: &
                                                                      snowncv, &
...
!
   ...
   real(kind=kind_phys), dimension( nlev , ncol , 2) :: &
                                                              qci
   real(kind=kind_phys), dimension( nlev , ncol , 4) :: &
                                                              qrs
   real(kind=kind_phys), dimension( nlev, 4)  :: &
                                                           rslope, &
                                                          rslope2, &
                                                          rslope3, &
                                                          rslopeb
...
!
   real(kind=kind_phys), dimension( nlev )   :: ncaut
   integer :: nu_c
   logical :: L_qc
...
!
   ...
   do k = ktop, kts, -1
     if(rh_mul(k)>1.) then
       ...
       hvalue = min(max(w1(i,k) + 0.5*sqrt(tke1(i,k)),0.),1.)
       ncact(k) = max(0.,htotal*hvalue)
...
!
  ...
  real(kind=kind_phys), dimension(kdim), &
        intent(inout   ) ::                                                    &
                                                                      ...
                                                                      rslope3, &
                                                                           vt
  real(kind=kind_phys)       ::  lamda, hvalue
  integer :: k
!-------------------------------------------------------------------------------
!
   do k = 1,ktop
...
!  size distributions: (x=mixing ratio, y=air density):
   contains 
   elemental real function lamdar2m(x, y, z)
       real, intent(in) :: x, y, z
       lamdar2m = exp(log(((pidnr * z) / (x * y))) * (0.33333333))
   end function lamdar2m
   elemental real function lamdar1m(x, y)
       real, intent(in) :: x, y
       lamdar1m = exp(log(((pidn0r) / (x * y))) * (0.25))
   end function lamdar1m
...
!
  ...
  real(kind=kind_phys), dimension(kdim),                                       &
   ...
   real(kind=kind_phys), dimension( kdim ) ::                                  &
                                                                       n0gfac
  real(kind=kind_phys)       ::  lamda, tc, hvalue
  integer :: k
!----------------------------------------------------------------
!-------------------------------------------------------------------------------
   n0gfac(:) = 0.0
...
!  size distributions: (x=mixing ratio, y=air density):
   contains 
   elemental real function lamdag(x, y, z)
       real, intent(in) :: x, y, z
       lamdag = exp(log(((pidn0g * z) / (x * y))) * (0.25))
   end function lamdag
...
!  size distributions: (x=mixing ratio, y=air density):
   contains 
   elemental real function lamdah(x, y, z)
       real, intent(in) :: x, y, z
       lamdah = exp(log(((pidn0h * z) / (x * y))) * (0.25))
   end function lamdah
...
!-------------------------------------------------------------------------------
  ...
  real(kind=kind_phys), dimension(kdim),                                       &
        intent(inout   ) ::                                                    &
                                                                      ...
                                                                      rslope2, &
                                                                      rslope3
  real(kind=kind_phys)       ::  lamda, tc, qmin
  integer ::  k
!-------------------------------------------------------------------------------
!
   do k = 1,ktop
...
!  size distributions: (x=mixing ratio, y=air density):
   contains 
   elemental real function lamdac(x, y, z)
       real, intent(in) :: x, y, z
       lamdac = exp(log(((pidnc * z) / (x * y))) * (0.33333333))
   end function lamdac
...
!  size distributions: (x=mixing ratio, y=air density):
   contains 
   elemental real function lamdas(x, y, z)
       real, intent(in) :: x, y, z
       lamdas = exp(log(((pidn0s * z) / (x * y))) * (0.25))
   end function lamdas
...
!
   ...
   real     ::  xj
!
   xj = min(max(c1xaut+c2xaut*d,1.d0), dble(nxaut))
   jx = min(xj, nxaut -1.)
